...
有关详细信息，请参见 link:https://docs.netapp.com/us-en/data-services-data-classification/task-health-monitor.html["监控数据分类的运行状况"^]。
...
数据分类支持优先处理映射和分类扫描以及仅映射扫描的功能，使您能够选择首先完成哪些扫描。在扫描开始期间和之前支持映射和分类扫描的优先级设置。如果选择在扫描正在进行时对扫描进行优先级排序，则会优先进行映射和分类扫描。
...
Data Classification 支持向 link:https://docs.netapp.com/us-en/console-setup-admin/task-monitor-cm-operations.html#audit-user-activity-from-the-bluexp-timeline["NetApp Console 审核页面"^] 发送分类事件（启动扫描时和扫描结束时）。
...
BlueXP classification 支持工作环境的批量操作。您可以选择启用映射扫描、启用映射和分类扫描、禁用扫描或在工作环境中跨卷创建自定义配置。如果您为单个卷进行选择，它将覆盖批量选择。要执行批量操作，请导航到**配置**页面并进行选择。
...
在进行仅地图扫描时，您可以优先考虑最重要的扫描。当您有多个工作环境并希望确保首先完成高优先级扫描时，此功能会有所帮助。
...
优先级仅限于 link:https://docs.netapp.com/us-en/data-services-data-classification/concept-classification.html["仅映射扫描"^]；无法用于映射和分类扫描。
...
* 激活审计日志收集的选项已被禁用。
* 在目录调查期间，不提供按目录计算个人身份信息 (PII) 数据数量的选项。请参阅 link:task-investigate-data.html["调查组织中存储的数据"]。
...
Red Hat Enterprise Linux 版本 8 和 9 不支持 Docker 引擎，此前 BlueXP classification 安装需要此引擎。BlueXP classification 现在支持在 RHEL 9.0、9.1 和 9.2 上使用 Podman 版本 4 或更高版本作为容器基础设施进行安装。如果您的环境需要使用最新版本的 RHEL，现在可以在使用 Podman 时安装 BlueXP classification（版本 1.26 或更高版本）。

目前，使用 RHEL 9.x 时，Data Classification 既不支持暗站点安装，也不支持分布式扫描环境（使用主扫描节点和远程扫描节点）。
...
"有访问权限的用户数量"过滤器现在支持存储在 SharePoint Online 存储库中的文件。此前，仅支持 CIFS 共享上的文件。此时，非基于 Active Directory 的 SharePoint 组不计入此筛选器。
