...
---
= 将 Active Directory 与 NetApp Data Classification 集成
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* 选择"查看所有权限"按钮时，您可以看到每个文件和目录的 link:task-investigate-data.html["完整文件权限"]。
...
对于 CIFS 卷/文件共享，如果要确保您的文件"上次访问时间"未被 Data Classification 扫描更改，则用户应具有写入属性权限。如果可能，我们建议将 Active Directory 配置的用户设置为组织中具有所有文件权限的父组的一部分。
...
. 在数据分类中，选择**配置**。
. 切换到 **Settings** 选项卡。
. 在 Active Directory 标题中，选择**添加**。
. 在"连接到 Active Directory"对话框中，输入 Active Directory 详细信息，然后选择*连接*。
+
...
如果需要修改 Active Directory 集成中的任何值，则必须在 Data Classification 中更新 Active Directory 配置。返回 Data Classification 中的 **Configuration** 菜单，然后切换到设置选项卡以进行更改。

您还可以通过选择 Active Directory 下的**删除**按钮来删除集成。
