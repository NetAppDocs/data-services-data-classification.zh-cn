...
. 从数据分类菜单中，选择*配置*。
. 在"配置"页面中找到 Google Cloud NetApp Volumes 系统。
...
.结果
Data Classification 开始扫描您在系统中选择的卷。Data Classification 完成初始扫描后，即可在合规性仪表板中查看结果。所需时间取决于数据量：几分钟到几小时。您可以从系统概述部分跟踪单个扫描的进度。Data Classification 会显示每次扫描的进度条。您还可以将鼠标悬停在进度条上，以查看扫描的文件数相对于卷中的文件总数。
